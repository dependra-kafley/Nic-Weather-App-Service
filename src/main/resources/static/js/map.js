...
const injectInlineSvg = async () => {
    const url = "meghalaya_districts.svg";
    const response = await fetch(url);
    if (!response.ok) {
        throw new Error(`Map SVG failed (${response.status}): ${url}`);
    }
...
const init = async () => {
    try {
        ...
        const msg = err && err.message ? err.message : "Could not load map.";
        districtDetailCard.innerHTML = `<span class="text-danger">${msg}</span>`;
        if (districtMapContainer) {
            districtMapContainer.innerHTML = `<p class="text-danger small mb-0">${msg}</p>`;
        }
    }
};
